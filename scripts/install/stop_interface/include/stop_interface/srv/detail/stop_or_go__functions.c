// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from stop_interface:srv/StopOrGo.idl
// generated code does not contain a copyright notice
#include "stop_interface/srv/detail/stop_or_go__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"

bool
stop_interface__srv__StopOrGo_Request__init(stop_interface__srv__StopOrGo_Request * msg)
{
  if (!msg) {
    return false;
  }
  // ittc
  // dist
  return true;
}

void
stop_interface__srv__StopOrGo_Request__fini(stop_interface__srv__StopOrGo_Request * msg)
{
  if (!msg) {
    return;
  }
  // ittc
  // dist
}

bool
stop_interface__srv__StopOrGo_Request__are_equal(const stop_interface__srv__StopOrGo_Request * lhs, const stop_interface__srv__StopOrGo_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // ittc
  if (lhs->ittc != rhs->ittc) {
    return false;
  }
  // dist
  if (lhs->dist != rhs->dist) {
    return false;
  }
  return true;
}

bool
stop_interface__srv__StopOrGo_Request__copy(
  const stop_interface__srv__StopOrGo_Request * input,
  stop_interface__srv__StopOrGo_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // ittc
  output->ittc = input->ittc;
  // dist
  output->dist = input->dist;
  return true;
}

stop_interface__srv__StopOrGo_Request *
stop_interface__srv__StopOrGo_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  stop_interface__srv__StopOrGo_Request * msg = (stop_interface__srv__StopOrGo_Request *)allocator.allocate(sizeof(stop_interface__srv__StopOrGo_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(stop_interface__srv__StopOrGo_Request));
  bool success = stop_interface__srv__StopOrGo_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
stop_interface__srv__StopOrGo_Request__destroy(stop_interface__srv__StopOrGo_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    stop_interface__srv__StopOrGo_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
stop_interface__srv__StopOrGo_Request__Sequence__init(stop_interface__srv__StopOrGo_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  stop_interface__srv__StopOrGo_Request * data = NULL;

  if (size) {
    data = (stop_interface__srv__StopOrGo_Request *)allocator.zero_allocate(size, sizeof(stop_interface__srv__StopOrGo_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = stop_interface__srv__StopOrGo_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        stop_interface__srv__StopOrGo_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
stop_interface__srv__StopOrGo_Request__Sequence__fini(stop_interface__srv__StopOrGo_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      stop_interface__srv__StopOrGo_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

stop_interface__srv__StopOrGo_Request__Sequence *
stop_interface__srv__StopOrGo_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  stop_interface__srv__StopOrGo_Request__Sequence * array = (stop_interface__srv__StopOrGo_Request__Sequence *)allocator.allocate(sizeof(stop_interface__srv__StopOrGo_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = stop_interface__srv__StopOrGo_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
stop_interface__srv__StopOrGo_Request__Sequence__destroy(stop_interface__srv__StopOrGo_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    stop_interface__srv__StopOrGo_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
stop_interface__srv__StopOrGo_Request__Sequence__are_equal(const stop_interface__srv__StopOrGo_Request__Sequence * lhs, const stop_interface__srv__StopOrGo_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!stop_interface__srv__StopOrGo_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
stop_interface__srv__StopOrGo_Request__Sequence__copy(
  const stop_interface__srv__StopOrGo_Request__Sequence * input,
  stop_interface__srv__StopOrGo_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(stop_interface__srv__StopOrGo_Request);
    stop_interface__srv__StopOrGo_Request * data =
      (stop_interface__srv__StopOrGo_Request *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!stop_interface__srv__StopOrGo_Request__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          stop_interface__srv__StopOrGo_Request__fini(&data[i]);
        }
        free(data);
        return false;
      }
    }
    output->data = data;
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!stop_interface__srv__StopOrGo_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


bool
stop_interface__srv__StopOrGo_Response__init(stop_interface__srv__StopOrGo_Response * msg)
{
  if (!msg) {
    return false;
  }
  // stop_flag
  return true;
}

void
stop_interface__srv__StopOrGo_Response__fini(stop_interface__srv__StopOrGo_Response * msg)
{
  if (!msg) {
    return;
  }
  // stop_flag
}

bool
stop_interface__srv__StopOrGo_Response__are_equal(const stop_interface__srv__StopOrGo_Response * lhs, const stop_interface__srv__StopOrGo_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // stop_flag
  if (lhs->stop_flag != rhs->stop_flag) {
    return false;
  }
  return true;
}

bool
stop_interface__srv__StopOrGo_Response__copy(
  const stop_interface__srv__StopOrGo_Response * input,
  stop_interface__srv__StopOrGo_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // stop_flag
  output->stop_flag = input->stop_flag;
  return true;
}

stop_interface__srv__StopOrGo_Response *
stop_interface__srv__StopOrGo_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  stop_interface__srv__StopOrGo_Response * msg = (stop_interface__srv__StopOrGo_Response *)allocator.allocate(sizeof(stop_interface__srv__StopOrGo_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(stop_interface__srv__StopOrGo_Response));
  bool success = stop_interface__srv__StopOrGo_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
stop_interface__srv__StopOrGo_Response__destroy(stop_interface__srv__StopOrGo_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    stop_interface__srv__StopOrGo_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
stop_interface__srv__StopOrGo_Response__Sequence__init(stop_interface__srv__StopOrGo_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  stop_interface__srv__StopOrGo_Response * data = NULL;

  if (size) {
    data = (stop_interface__srv__StopOrGo_Response *)allocator.zero_allocate(size, sizeof(stop_interface__srv__StopOrGo_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = stop_interface__srv__StopOrGo_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        stop_interface__srv__StopOrGo_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
stop_interface__srv__StopOrGo_Response__Sequence__fini(stop_interface__srv__StopOrGo_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      stop_interface__srv__StopOrGo_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

stop_interface__srv__StopOrGo_Response__Sequence *
stop_interface__srv__StopOrGo_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  stop_interface__srv__StopOrGo_Response__Sequence * array = (stop_interface__srv__StopOrGo_Response__Sequence *)allocator.allocate(sizeof(stop_interface__srv__StopOrGo_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = stop_interface__srv__StopOrGo_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
stop_interface__srv__StopOrGo_Response__Sequence__destroy(stop_interface__srv__StopOrGo_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    stop_interface__srv__StopOrGo_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
stop_interface__srv__StopOrGo_Response__Sequence__are_equal(const stop_interface__srv__StopOrGo_Response__Sequence * lhs, const stop_interface__srv__StopOrGo_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!stop_interface__srv__StopOrGo_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
stop_interface__srv__StopOrGo_Response__Sequence__copy(
  const stop_interface__srv__StopOrGo_Response__Sequence * input,
  stop_interface__srv__StopOrGo_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(stop_interface__srv__StopOrGo_Response);
    stop_interface__srv__StopOrGo_Response * data =
      (stop_interface__srv__StopOrGo_Response *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!stop_interface__srv__StopOrGo_Response__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          stop_interface__srv__StopOrGo_Response__fini(&data[i]);
        }
        free(data);
        return false;
      }
    }
    output->data = data;
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!stop_interface__srv__StopOrGo_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
